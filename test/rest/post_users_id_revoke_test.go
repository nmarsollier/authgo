...
func TestPostUserRevokeHappyPath(t *testing.T) {
	...
	req, w := TestPostRequest("/users/"+userData.ID.Hex()+"/revoke", revokePermissionBody{Permissions: []string{"user"}}, tokenString)
	r.ServeHTTP(w, req)
...
func TestPostUserRevokeFindUserError_1(t *testing.T) {
	...
	req, w := TestPostRequest("/users/"+userData.ID.Hex()+"/revoke", revokePermissionBody{Permissions: []string{"people"}}, tokenString)
	r.ServeHTTP(w, req)
...
func TestGetUserSignOutMissingTokenHeader(t *testing.T) {
	...
	req, w := TestPostRequest("/users/123/revoke", revokePermissionBody{Permissions: []string{"people"}}, "")
	r.ServeHTTP(w, req)
