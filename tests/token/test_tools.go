...
func ExpectTokenFindOne(coll *db.MockMongoCollection, tokenData *token.Token, times int) {
...
func ExpectTokenAuthFindOne(t *testing.T, coll *db.MockMongoCollection, tokenData *token.Token) {
	coll.EXPECT().FindOne(gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(
