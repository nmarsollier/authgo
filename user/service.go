...
func (s serviceImpl) SignUp(user *SignUpRequest) (string, error) {
	...
	newUser, err := getDao().Insert(newUser)
	if err != nil {
	...
	token, err := getService().Create(newUser.ID)
	if err != nil {
		return "", nil
	}
...
func (s serviceImpl) SignIn(login string, password string) (string, error) {
	user, err := getDao().FindByLogin(login)
	if err != nil {
		return "", err
	}
	...
	token, err := getService().Create(user.ID)
	if err != nil {
		return "", nil
	}
...
func (s serviceImpl) ChangePassword(userID string, current string, newPassword string) error {
	user, err := getDao().FindByID(userID)
	if err != nil {
		return err
	}
...
func (s serviceImpl) Granted(userID string, permission string) bool {
	usr, err := getDao().FindByID(userID)
	if err != nil {
		return false
	}
